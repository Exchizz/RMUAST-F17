...
for line in f:
	...
	if count > 20:
		for i in range(1,20):
			dataPitch[i - 1] = dataPitch[i]
			dataRoll[i - 1] = dataRoll[i]
		dataPitch[-1] = pitch
		dataRoll[-1] = roll
	else:
		dataPitch.append(pitch)
		dataRoll.append(roll)

	print len(dataPitch)
	avgPitch = 0.0
	avgRoll = 0.0

	for i in range(len(dataPitch)):
		avgPitch += dataPitch[i]
		avgRoll += dataRoll[i]

	print 'avgPitch: ', avgPitch
	avgPitch /= len(dataPitch)
	avgRoll /= len(dataRoll)
	...
	plotDataPitchAvg.append (avgPitch*180.0/pi)
	plotDataRollAvg.append (avgRoll*180.0/pi)
